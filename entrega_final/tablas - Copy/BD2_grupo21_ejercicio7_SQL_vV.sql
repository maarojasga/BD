...
*/
create or alter procedure entrega_pedido_estado
   (@id bigint)
as
BEGIN
   set nocount on
...
create or alter procedure rechazo_pedido_estado
   (@id bigint)
as
BEGIN
   set nocount on
...
create or alter procedure en_proceso_pedido_estado
   (@id bigint)
as
BEGIN
   set nocount on
...
*/
create or alter procedure insert_premio_empleado
   (
...
BEGIN
   ...
   if (SELECT COUNT(*)
   FROM PREMIOS) > 5
        DELETE FROM PREMIOS WHERE PREMIO_ID =  (SELECT MIN(PREMIO_ID)
   FROM PREMIOS )
END
go
...
create or alter procedure insert_premio_empleado_vendedor
   (
   @PREMIO_NOMBRE VARCHAR(100),
   @PREMIO_VALOR BIGINT,
...
CREATE OR ALTER TRIGGER historia_empleados_insert ON EMPLEADOS
FOR DELETE AS 
...
CREATE OR ALTER TRIGGER historia_PRoveedores_insert ON PROVEEDORES
FOR DELETE AS 
    INSERT INTO REGISTRO_HISTORICO
   (REGISTRO_HISTORICO_MENSAJE)
VALUES
   ('DELETE EN PROVEEDORES')
    GO
CREATE OR ALTER TRIGGER historia_productos_insert ON PRODUCTOS
FOR DELETE AS 
    INSERT INTO REGISTRO_HISTORICO
   (REGISTRO_HISTORICO_MENSAJE)
VALUES
   ('DELETE EN PRODUCTOS')
    GO
CREATE OR ALTER TRIGGER historia_clientes_insert ON CLIENTES
FOR DELETE AS 
    INSERT INTO REGISTRO_HISTORICO
   (REGISTRO_HISTORICO_MENSAJE)
VALUES
